...
const Portfolio = () => {
  // Profile image state and fetch logic
  const [profileImageUrl, setProfileImageUrl] = useState<string>('');
  const [imageLoading, setImageLoading] = useState(true);
  const [imageError, setImageError] = useState(false);

  useEffect(() => {
    const imageUrl = "https://media.licdn.com/dms/image/v2/D4E03AQHOkV4A1TEuqA/profile-displayphoto-shrink_800_800/profile-displayphoto-shrink_800_800/0/1687341639373?e=1755734400&v=beta&t=gPYTlYiubfvQlQBQuXCP-aPVxjYyWWQXBL7NlJyKfgs";
    const cacheKey = 'profileImageCache';
    const cacheTimeKey = 'profileImageCacheTime';
    const now = Date.now();
    const oneDay = 24 * 60 * 60 * 1000;
    const cachedImage = localStorage.getItem(cacheKey);
    const cachedTime = localStorage.getItem(cacheTimeKey);
    if (cachedImage && cachedTime && now - parseInt(cachedTime) < oneDay) {
    ...
    fetch(proxyUrl)
      ...
      .catch(() => {
        setImageError(true);
        setImageLoading(false);
        setProfileImageUrl('/profile.jpg'); // fallback to local image
      });
  }, []);
  const [state, handleSubmit] = useForm('mblyvago');
          ...
          <div className="animate-fade-in">
            <div className="w-32 h-32 mx-auto mb-8 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 p-1">
              <div className="w-full h-full rounded-full bg-white dark:bg-gray-800 flex items-center justify-center text-4xl">
                ...
                >
                  {imageLoading ? (
                    <div className="w-full h-full flex items-center justify-center animate-pulse bg-gradient-to-r from-blue-500 to-purple-500 rounded-full">
                      <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    </div>
                  ) : profileImageUrl && !imageError ? (
                    <img
                      src={profileImageUrl}
                      alt="Profile - Click to view LinkedIn"
                      className="rounded-full w-full h-full object-cover hover:scale-105 transition-transform duration-200 cursor-pointer"
                      onError={() => {
                        setImageError(true);
                        setProfileImageUrl('/profile.jpg');
                      }}
                    />
                  ) : (
                    <div className="w-full h-full rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center text-white text-2xl font-bold hover:scale-105 transition-transform duration-200 cursor-pointer">
                      GH
                    </div>
                  )}
                </a>
